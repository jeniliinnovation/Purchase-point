...
const BuyerTopbar = ({ setActiveTab, toggleSidebar }) => {
  ...
  return (
    <div className="h-20 bg-white/80 backdrop-blur-md border-b border-gray-100 flex items-center justify-between px-4 md:px-8 sticky top-0 z-40 lg:ml-64 transition-all">
      {/* Mobile Toggle */}
      <button 
      ...
      <div className="flex-1 max-w-xl">
        <div className="relative group">
          ...
          <input 
            type="text" 
            placeholder="Search sourcing nodes, RFQs, or suppliers..."
      ...
      <div className="flex items-center gap-6">
        ...
        >
          {user?.avatar ? (
            ...
            <div className="w-10 h-10 bg-gradient-to-tr from-upwork-dark to-upwork-green rounded-xl flex items-center justify-center text-white font-bold uppercase shadow-sm">
              {user?.name?.charAt(0) || 'B'}
            </div>
          )}
          <div className="hidden md:block">
            <p className="text-sm font-black text-upwork-dark leading-tight uppercase truncate max-w-[120px]">
              {user?.name || 'Buyer Node'}
            </p>
            <p className="text-[10px] text-gray-400 font-bold uppercase tracking-wider">
              {user?.company || 'Authorized Buyer'}
            </p>
          </div>
